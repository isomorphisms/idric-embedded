# AVR instruction inventory for ATmega targets
...
This branch intentionally records the **complete 8-bit AVR instruction-name union** rather than pretending every ATmega implements every historical AVR instruction. Once a concrete ATmega device is selected, add its exact core/version and mark availability against this union.

## Complete instruction-description inventory

The current manual contains the following 124 instruction-description entries, in manual order. Entries such as `LD/LDD`, `ST/STD`, `LDS (AVRrc)`, and `SPM` have distinct encodings/forms inside the named entry and must remain distinct in any machine-readable encoding table.
...
## Aliases versus independent encodings

Do not count spelling convenience as an extra machine encoding. In particular several familiar mnemonics are aliases/specializations of more general encodings (`CLR`, `LSL`, `ROL`, `TST`, `CBR`, `SBR`, flag set/clear and conditional-branch spellings). Preserve both assembler-visible mnemonic and underlying encoding identity in a later TSV/JSON table.

## Branch completeness rule

This is the complete mnemonic/description union from DS40002198C, but **ATmega device applicability is not yet pinned** because the branch is named for the product family, not one device. The next architecture-data layer should be a machine-readable table with one row per encoding/form and columns for AVR/AVRe/AVRxm/AVRxt/AVRrc plus the selected ATmega device.

Idriç support must be tracked separately; an unsupported instruction remains part of this architecture inventory.
